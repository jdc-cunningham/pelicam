...
Will note I just turned this camera on from being fully charged, will see how long it lasts, granted most of the time it's just sitting in "standby" no live camera passthrough

The screen auto-dims itself too it's interesting after not being used for a bit, not sure how long

I've been thinking about it and I really need to shoot in manual

So I'm going to do the OSD options next
...
There might be some other settings I'm not thinking of.

Exposure is one

Also been thinking about using focus algorithms, there is the FocusFOM which is not documented in Picamera2's library from what I've seen but is a thing apparently

Although I believe you look at it after you've taken a picture

Otherwise you'd run an algo on your frame sample

This guy's videos are good Simon d'Entremont

Gonna see if he has videos on the topics above to put those into the camera

Also I'm trying to not spend money on anything, just pay off debt

So I'm focusing on software projects or hardware projects where I already have the parts

I do want to build another camera with another DSI display, just a simpler body/better to hold

11:04 AM

Doing some reading

So it seems that the exposure time is normal, it's in milliseconds though so have to convert it

Gain... I'm not sure how to deal with that, the min/max range for the IMX477 is 1 to 16... so idk I guess you could fake the range with 100, 200, 400, that kind of thing

Either way I need to build a basic menu interface that will increment/decrement a number and display what it is

11:28 AM

I could make a menu but I think it makes more sense to just change the values directly

So you'd have the overlayed settings/text on the camera passthrough, you'd tap it to engage it (not zoom in)

It'd have a border drawn around it to tell you it's active, then you use the up/down arrows (d-pad) to change it

12:45 PM

Making progress, I just went through and did these seconds to microseconds conversion

Found a chart online of standard shutter speed fractional values

Working on the tap/edit part right now

1:06 PM

It's funny this camera is warm to the touch

1:21 PM

Rebooted maybe this is the limit of this camera for battery usage

Seems like 2 hrs 40 mins

4:55 PM

Back from the gym and just ate

Feel kind of lazy but I want to add the aperture overlay and white balance

5:10 PM

Trying to fix this index problem

My mind is just swimming, all over the place

6:17 PM

Alright I've got the programmable settings, they seem to be working together

When the scene is very bright ISO doesn't seem to do anything, although it is working, tested in a dark area
...
Alright, trying to update the code and be ready to go to a new park in about 4 hrs

Should be plenty of time

What I want to address:

- [x] rework zoom pan using ScalarCrop
- [x] make basic menu so shutter delay can be triggered
- [x] add IMU shake anime girl face gimmick

I really should be able to set the shutter speed, maybe I'll investigate it but I need a menu system that does more than on/off

It should be bright enough outside with the shutter delay and tripod to still get good pictures since I'm using a zoom lens the Computar 8-48mm

11:58 AM

Alright, ran into some problems but we've got a funcitonal 1x 3x 5x pan-zoom

Now to add the basic menu and shutter delay option

I'm tempted to sneak in the shutter speed control but I need to learn more about it too not only from coding but photography perspective

It should be bright enough outside where the camera automatically performs faster/takes pictures faster but the vibration on the tripod is apparent even on my small tripod I'm using while programming

12:02 PM

I'm not going to make a dedicate page, I'll just put a toggle-able button on the existing view files page

12:14 PM

Alright got a basic shutter delay toggle

1:05 PM

Alright got the anime girl scared face tied to IMU
...
Got some feedback regarding ScalarCrop, I'll need to rework the camera pan-zoom code to use that.

Damn I feel mentally drained though, maybe all the candy I ate/soda

But I'll re-work this pan-zoom process and make the menu for delayed shutter

I will later do the overlay on the camera/do settings like ISO, shutter speed, etc...
...
Alright I have a goal now, I'm going to go to a new park this coming Sunday eg. 02/22/2026

I will take the Computar zoom lens with me which turns out to be a pretty dope lens once I figured out how to get it to focus (remove CS adapter)

- [ ] make a basic menu page for toggling shutter delay
  - [ ] add "settings" option to "View Images" menu page
  - [ ] click that and open new Settings page
  - [ ] in the Settings page is a clickable text that says "Shutter delay: Off" and toggled it's "Shutter delay: On" with a hard-coded 5 second delay
- [x] update zoom needs to be more than 3x
  - not the solution but working
- [x] fix the freezing problem when zooming
  - zoom in, hit back, frozen

7:41 PM

Says null preview exception

8:24 PM

Alright... the 5X video configuration doesn't work for some reason keeps saying "no such process" when it tries to start the camera

So I guess I will have to figure out that ScalarCrop thing mentioned to me

8:32 PM

At a quick glance it looks like what I implemented through PIL which I was advised not to use since it's lower performance than with CV since I'm already using CV

I can't get the config to work for:

```
self.video_config_3x = self.picam2.create_video_configuration(
  main={"size": (3200, 2400), "format":"RGB888"}
)
```

Event without color profile specified, it must be the size

The modes on the camera output it has a 2028x1520 then 4056x3040

Anyway the framerate would be so slow it's not worth it

I'll just upscale the image for 5x
...
So there is this problem with git where it just gets corrupted

Maybe it's an SD card problem specificially or something but happens on RPis

Anyway that's why this is branch-3

can't remember and branch-2 was just switching streaming options, I was trying taking pictures vs. the stream and the stream is much faster

10:38 AM

Distracted

I need to add the panning piece, I've got a basic zoom in

10:43 AM

I didn't realize how massive 4056x3040 is against 640x480

It's almost 7 time as large

11:15 AM

Alright at this point I've got software I can use, added ability to pan, I do wonder if 3x is enough
...
I was originally developing this in the JDC34 camera repo but ran into problems with paths

So just going to keep going here to get it done
...
Here you can see how it'll work

<img src="./devlog-mages/zoom-pan-plan.png"/>

Will try this resolution for now, I'm curious if the lag will be perceptible

The bigger image you take/sample the slower the refresh rate gets

Also I thought I was just going to do it through touchscreen alone but maybe it makes sense to combine physical and touch

I could do double-tap to zoom in but need to deal with timing

I'll start with the image clicking/viewing first

### 02/09/2026

8:31 PM

Side tracked a little

I did take the camera out first time, I definitely have to work on the touch part and image rendering more even though it was a quick "get it done" situation

Also some weird issue where the display showed horizontal colorful bars... I'm not sure what it was, thought it was power related but the camera kept going after reboot
...
At this point I have the basic software written where it can do:
- live passthrough
- button interface
- small OLED interface
- show last 8 photos taken

There are still some things I need to address but it's usable for now.

The pass through color is off, blues look green for some reason

The touch coordinate system I did a poor job on so it's not right but works

I initially recorded the software dev process but it was over 5 hours of video so it's like nvm... I'll just do the highlights

What's also concerning is the OLED does not render correctly if the power is low I guess... I may have to switch to a 3A step up converter
